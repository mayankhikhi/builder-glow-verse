...
export const BookingDialog = ({ children }: { children: ReactNode }) => {
  ...
  const totalPrice = useMemo(() => {
    return (
      selectedServiceIds?.reduce(
        (sum, id) => sum + (ALL_SERVICES.find((s) => s.id === id)?.price ?? 0),
        0,
      ) ?? 0
    );
  }, [selectedServiceIds]);
  ...
  const onSubmit = handleSubmit((values) => {
    ...
    toast.success(
      "Appointment booked! You'll receive a confirmation email after connecting email service.",
    );
    setOpen(false);
    reset();
  });
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                {...register("name")}
                required
                className="w-full rounded-md bg-muted/40 px-3 py-2 border"
